...
onMounted(async () => {
    ...
    try {
        const planResponse = await axios.get(`http://127.0.0.1:3000/plans/${route.params.id}`, {
            headers: { Authorization: `Bearer ${token}` }
        });

        // Unpack plan data ad nauseum
        plan.value = planResponse.data;
        planTitle.value = plan.value.title;
        startDate.value = plan.value['start_date'];
        endDate.value = plan.value['end_date'];
        

        const mealsResponse = await axios.get('http://127.0.0.1:3000/recipes', {
            headers: { Authorization: `Bearer ${token}` }
        });
    ...
    <div class="container-lg mt-5">
        <div class="row justify-content-center">
            <div class="col-md-6 col-12">
                <h1 class="fw-bold">Edit plan: {{ planTitle }}</h1>
