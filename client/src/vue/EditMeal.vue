...
onMounted(async () => {
    try {
        ...
        prevName.value = response.data.name;
        mealName.value = response.data.name;
        tags.value = response.data.tags;
        ingredients.value = response.data.ingredients;

        for (let i = 0; i < response.data.steps.length; i++) {
            steps.value += response.data.steps[i];

            if (i < response.data.steps.length - 1) steps.value += '\n';
        }
    } catch (err) {
        errorMessage.value.push('Error retrieving meal details. Please try again.');
        console.error(err);
    }
});
...
const submitMeal = async () => {
    ...
    try {
        const response = await axios.put(`http://127.0.0.1:3000/recipes/${route.params.id}`, formData, {
            headers: {
    ...
    <div class="container-lg mt-5">
        <div class="row justify-content-center">
            <div class="col-md-6 col-12">
                <h1 class="fw-bold">Edit meal: {{ mealName }}</h1>
